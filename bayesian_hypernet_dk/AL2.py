...
from bayesian_hypernet_dk.acquisition_functions import bald, max_ent, var_ratio, mean_std
from bayesian_hypernet_dk.BHNs import HyperCNN
from bayesian_hypernet_dk.helpers import split_train_pool_data, get_initial_training_data
# TODO:
...
def train_epoch(train_func,predict_func,X,Y,Xt,Yt,
                lr0=0.1,lrdecay=1,BATCH_SIZE=20):
...
def train_model(train_func,predict_func,X,Y,Xt,Yt,
                lr0=0.1,lrdecay=1,BATCH_SIZE=20,epochs=50):
    ...
    for e in range(epochs):
        ...
        num_steps = N/BATCH_SIZE + int(N%BATCH_SIZE > 0)
...
def active_learning(num_acquisitions):
    ...
    for i in range(num_acquisitions):
        ...
        if acq == 'bald':
            ...
            acq_inds = sort_values.argsort()[-acq_size:][::-1]
            #print acq_inds.shape # 10
            #assert False
            ...
            acq_inds = a_1d.argsort()[-acq_size:][::-1]
            
        if acq == 'random':
            # acquire a random subset of the pool
            acq_inds = np.random.choice(range(POOL_SIZE), acq_size, replace=False)
        ...
        # acquire points
        acquired_X = X_pool_Dropout[acq_inds, :, :, :]
        acquired_Y = y_pool_Dropout[acq_inds] 
        # delete 2000 points
        ...
        delete_Pool_X_Dropout = np.delete(X_pool_Dropout, (acq_inds), axis=0)
        delete_Pool_Y_Dropout = np.delete(y_pool_Dropout, (acq_inds), axis=0)
        # add 1990 points back 
        pool_x = np.concatenate((pool_x, X_pool_Dropout), axis=0)
        pool_y = np.concatenate((pool_y, y_pool_Dropout), axis=0)
        # add acquired points
        train_x = np.concatenate((train_x, acquired_X), axis=0)
        train_y = np.concatenate((train_y, acquired_Y), axis=0).astype('float32')
        #print pool_x.shape, acquired_X.shape, train_x.shape
        #assert False
...
def main():
    ...
    valid_all_accuracy = np.zeros(shape=(num_acquisitions+1, num_experiments))
    test_all_accuracy = np.zeros(shape=(num_acquisitions+1, num_experiments))
    
    for i in range(num_experiments):
        
        test_accuracy, valid_accuracy = active_learning(num_acquisitions)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lbda',default=1,type=float) # hnet (PRIOR?)
    ...
    if args.PRIOR=='log_normal':
        ...
        PRIOR = log_laplace
    size = 10000#max(10,min(50000,args.size))
