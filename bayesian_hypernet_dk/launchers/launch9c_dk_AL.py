...
if subprocess.check_output("hostname").startswith("hades"):
    ...
    exp_script = ' $HOME/copy_to_helios_BayesianHypernetDK/bayesian_hypernet_dk/active_learning.py '
else: # TODO: SLURM
...
line = [ "--model_type=CNN --dropout=1" ]
line = [" " + ss + " " for ss in line]

grid = [] 
grid += [["acq", ['numerically_stable_bald']]]#, 'max_ent', 'var_ratio']]]
grid += [["epochs", [50]]]#, 100]]]#, 200]]]
# TODO: rename "training_method" (??)
grid += [["params_reset", ['random', 'none', 'pretrain_and_warm_start']]]
# 
