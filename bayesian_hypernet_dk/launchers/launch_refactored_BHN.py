...
def combine_grid_line_search(grid, line=None):
    if line is None:
        return grid
...
if subprocess.check_output("hostname").startswith("hades"):
    ...
    exp_script = ' $HOME/BayesianHypernetDK/bayesian_hypernet_dk/refactored_BHN.py '


job_prefix += exp_script

grid = [] 
grid += [["coupling_dim", [50, 200, 800]]]
grid += [["convex_combination", ['0', '.0000001', '1', '-1']]]
#grid += [["coupling", [4, 16]]]
grid += [["uncoupled_init", [0, 1, 2, 3]]]
grid += [["save", ['1']]]
#grid += [["test_eval", ['1']]]

search = combine_grid_line_search(grid_search(grid), line=None)
...
if launch:
    for nn, job_str in enumerate(job_strs):
        os.system(job_str)
        #print job_str
        if nn % 8 == 7:
            time.sleep(2000)
