...
class DK_BHN(SaveLoadMIXIN):
    ...
    #def _get_hyper_net(self):
        ...
        if self.coupling:
            layer_temp = CoupledWNDenseLayer(h_net,coupling_dim, uncoupled_init=uncoupled_init)
            h_net = IndexLayer(layer_temp,0)
            logdets_layers.append(IndexLayer(layer_temp,1))
        ...
        if self.convex_combination:
            layer_temp = ConvexBiasLayer(h_net, upweight_primary=self.convex_combination)
            h_net = IndexLayer(layer_temp,0)
            logdets_layers.append(IndexLayer(layer_temp,1))
...
if 1:#__name__ == '__main__':
    ...
    parser.add_argument('--coupling_dim',default=200,type=int)  
    ...
    parser.add_argument('--num_train',default=400, type=int)
    parser.add_argument('--perdatapoint',default=0,type=int)
    parser.add_argument('--plot_every',default=20,type=int)
...
if 1:
    ...
    X = train_x[:num_train]
    Y = train_y[:num_train]
    Xva = valid_x[:num_train]
    Yva = valid_y[:num_train]
    ...
    records['kl'] = kl
    records['acc'] = acc
    records['val_loss'] = val_loss
    records['val_nll'] = val_nll
    records['val_kl'] = val_kl
    ...
    for step in range(num_epochs):
        ...
        if step % plot_every == 0:
            ...
            print "                                                       ", time.time() - t0, "                    ", val_loss_, val_acc[-1]
            if 0:#plot:
                plt.clf()
                plt.subplot(121)
        ...
        np.save(save_path + 'records' + '.npy', records)
